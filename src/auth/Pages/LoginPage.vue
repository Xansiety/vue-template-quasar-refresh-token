    ...
    <q-card square class="shadow-6" style="width:50vh;height:450px;">
      <q-form class="q-px-sm q-pt-md" @submit="onSubmit">
        ...
        <q-card-section class="q-mt-md">
          <q-input ref="usuario" class="q-mb-md" label="Usuario" outlined clearable v-model="credentials.usuario"
            type="text" :rules="[val => val && val.length > 0 || 'Campo obligatorio']">
            <template v-slot:prepend>
              <q-icon name="person" />
            </template>
          </q-input> 
          <q-input ref="password" label="Contraseña" outlined clearable v-model="credentials.password" type="text"
        ...
        <q-card-actions class="q-px-md">
          <q-btn unelevated size="lg" color="secondary" type="submit" class="full-width text-white" label="Ingresar" />
        </q-card-actions>
      </q-form>
    </q-card>
  </div>
